...
function submitProposal() {
  ...
  setTimeout(() => {
    ...
    // Persist proposal into localStorage DB (manager/supervisor pages read from it)
    if (typeof pblSubmitProposal === 'function') {
      const roll = sessionStorage.getItem('pbl_roll_number') || '';
      const name = sessionStorage.getItem('pbl_name') || '';
      const program = sessionStorage.getItem('pbl_program') || '';
      const email = sessionStorage.getItem('pbl_email') || '';

      const res = pblSubmitProposal({
        title: document.getElementById('projTitle')?.value || '',
        dept: document.getElementById('projDept')?.value || '',
        category: document.getElementById('projCategory')?.value || '',
        supervisor: document.getElementById('projSupervisor')?.value || '',
        semester: document.getElementById('projSemester')?.value || '',
        abstract: document.getElementById('projAbstract')?.value || '',
        objectives: document.getElementById('projObjectives')?.value || '',
        methodology: document.getElementById('projMethodology')?.value || '',
      ...
      if (!res?.ok) {
        showAlert('Could not save proposal. Please try again.', 'danger');
        return;
      }

      // Update reference ID in success modal if present
      const ref = document.querySelector('#successModal strong.text-primary');
      if (ref) ref.textContent = `PBL-${new Date().getFullYear()}-${String(res.proposal.id).slice(-4)}`;
    }
