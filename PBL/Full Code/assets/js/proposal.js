...
function displayFile(file) {
  ...
  window._selectedFileObject = file;
...
function clearFile(e) {
  ...
  window._selectedFileObject = null;
}
...
function submitProposal() {
  ...
  formData.append('dept', document.getElementById('projDept')?.value || '');
  formData.append('category', document.getElementById('projCategory')?.value || '');
  formData.append('semester', document.getElementById('projSemester')?.value || '');
  formData.append('abstract', document.getElementById('projAbstract')?.value || '');
  formData.append('objectives', document.getElementById('projObjectives')?.value || '');
  formData.append('methodology', document.getElementById('projMethodology')?.value || '');
  formData.append('tools', document.getElementById('projTools')?.value || '');
  
  if (window._selectedFileObject) {
    formData.append('proposal_file', window._selectedFileObject);
  }

  fetch('../../api/student/submit_proposal.php', {
    method: 'POST',
    body: formData
  })
  .then(res => res.json())
  .then(res => {
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="bi bi-send me-1"></i> Submit Proposal';
    }

    if (res.success) {
      const ref = document.querySelector('#successModal strong.text-primary');
      if (ref) ref.textContent = `PBL-${new Date().getFullYear()}-${String(res.proposal_id).padStart(4, '0')}`;
      
      const modal = new bootstrap.Modal(document.getElementById('successModal'));
      modal.show();
    } else {
      showAlert(res.message || 'Could not save proposal.', 'danger');
    }
  })
  .catch(err => {
    if (btn) {
