...
function loadUserInfo() {
  const session = window.__pblSession || null;
  const name = (session?.name || sessionStorage.getItem('pbl_name') || 'User');
  const role = (session?.role || sessionStorage.getItem('pbl_role') || '');
  ...
  if (roleLabel) {
    const labels = { student: 'Student', supervisor: 'Supervisor', pbl_manager: 'PBL Manager', manager: 'PBL Manager', evaluator: 'Evaluator' };
...
/* --- Logout --- */
function logout() {
  fetch(pblUrlFromRoot('api/auth/logout.php'), { method: 'POST' }).catch(function () {});
  sessionStorage.clear();
  window.location.href = pblUrlFromRoot('index.php');
}
...
function requireAuth() {
  return fetch(pblUrlFromRoot('api/auth/check_session.php'))
...
function injectChangePasswordLink() {
  ...
  a.href = pblUrlFromRoot('pages/shared/change-password.php');
